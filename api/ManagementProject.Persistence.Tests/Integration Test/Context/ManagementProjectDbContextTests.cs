...
namespace ManagementProject.Persistence.Tests.Integration_Test.Context
{
    [TestClass]
    public class ManagementProjectDbContextTests
    {
        ...
        public async Task GetSchoolsAsync_WhenDatabaseIsEmpty_ReturnsEmptyList()
        {
            ...
            List<School> listSchools = null;
            // Act
            using (var inMemoryContext = new ManagementProjectDbContext(_inMemoryOptionsEmptyDb))
            {
                listSchools = await inMemoryContext.Schools?.ToListAsync();
            }
            // Assert
